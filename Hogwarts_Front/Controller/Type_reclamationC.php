...
Class Type_reclamationC {
    ...
    function getType_reclamationbyID($id)
       ...
       function ajouterType_reclamation($type_reclamation)
        ...
        public function afficherAbsences($Id_reclamation){
            $config = config::getConnexion();
            try{
                $query = $config->prepare(
                    'SELECT * FROM absence where type_reclamation = :id'
                );
                $query->execute([
                    'id' => $Id_reclamation
                ]);
        ...
        public function afficherNotes($Id_reclamation){
            $config = config::getConnexion();
            try{
                $query = $config->prepare(
                    'SELECT * FROM rec_note where type_reclamation = :id'
                );
                $query->execute([
                    'id' => $Id_reclamation
                ]);
        ...
        public function afficherAutres($Id_reclamation){
            $config = config::getConnexion();
            try{
                $query = $config->prepare(
                    'SELECT * FROM rec_autre where type_reclamation = :id'
                );
                $query->execute([
                    'id' => $Id_reclamation
                ]);
